...
class ActingOfficerListInput(BaseModel):
    employee_name: str | None = Field(
        None,
        description="Фамилия сотрудника, чьих ИО нужно найти (по умолчанию - текущий пользователь).",
    )
...
def create_acting_officer_tools(deps: AppDeps) -> list[StructuredTool]:
    ...
    async def acting_officer_list_tool(
        ...
        try:
            if employee_name:
                emp = await deps.employee_client.find_by_last_name_fts(
                    token, employee_name
                )
                target_id = emp.id
            else:
            ...
            ios = await deps.employee_acting_client.get_acting_for_target(
                token, target_id
            )
            return {
                "status": "success",
            ...
            return {
                "status": "error",
                "message": f"Ошибка при получении списка ИО: {e}",
            }
