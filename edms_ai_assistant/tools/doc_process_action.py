...
from edms_ai_assistant.domain.document import SimpleProcessAction, ProcessActionWithSign
...
class ProcessActionInput(BaseModel):
    action_type: DocumentProcessType = Field(..., description="Тип действия: AGREEMENT (Согласовать), SIGNING (Подписать), STATEMENT (Утвердить).")
    result: bool = Field(True, description="Результат действия (True - положительный, False - отрицательный/отклонить).")
...
def create_doc_process_action_tool(deps: AppDeps) -> StructuredTool:
    ...
    async def doc_process_action(
        ...
        try:
            ...
            return {"status": "error", "message": f"Ошибка авторизации или контекста: {e}"}

        try:
            # 1. Получаем процесс, чтобы найти текущий этап
            process = await deps.document_process_client.get_process(token, UUID(document_id))
            if not process or not process.current_id:
                return {"status": "error", "message": "Документ не находится в активном процессе."}

            current_item = process.current
            if not current_item or current_item.type != action_type:
                expected = action_type.value if hasattr(action_type, 'value') else str(action_type)
                actual = current_item.type.value if current_item and hasattr(current_item.type, 'value') else str(current_item.type if current_item else 'None')
                return {
                    "status": "error",
            ...
            if action_type == DocumentProcessType.AGREEMENT:
                body = SimpleProcessAction(result=result, employee_id=user.id, comment=comment)
                await deps.document_process_client.agreement(token, UUID(document_id), process.current_id, body)
            elif action_type == DocumentProcessType.SIGNING:
                body = ProcessActionWithSign(result=result, employee_id=user.id, comment=comment)
                await deps.document_process_client.signing(token, UUID(document_id), process.current_id, body)
            elif action_type == DocumentProcessType.STATEMENT:
                body = ProcessActionWithSign(result=result, employee_id=user.id, comment=comment)
                await deps.document_process_client.statement(token, UUID(document_id), process.current_id, body)
            elif action_type == DocumentProcessType.REVIEW:
                body = SimpleProcessAction(result=result, employee_id=user.id, comment=comment)
                await deps.document_process_client.review(token, UUID(document_id), process.current_id, body)
            else:
                return {"status": "error", "message": f"Действие {action_type} пока не поддерживается через чат."}
            ...
            return {"status": "error", "message": f"Ошибка при выполнении действия: {exc}"}
