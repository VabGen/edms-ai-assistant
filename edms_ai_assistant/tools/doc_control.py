...
async def doc_control(
    ...
    """Управляет контролем документа: поставить, отредактировать, снять или удалить.

    Действия (action):
    - set: поставить на контроль (обязательно нужны control_employee_id и control_type_name)
    - edit: отредактировать существующий контроль
    - remove: снять с контроля
    - delete: удалить запись контроля
    - get: получить текущий контроль

    Если при action='set' не указан контролёр или тип контроля, инструмент вернёт статус 'need_input'
    со списком доступных вариантов. В этом случае нужно спросить пользователя и вызвать инструмент повторно.

    Args:
        action: set | edit | remove | delete | get. По умолчанию 'set'.
        date_control_end: Дата окончания контроля в формате YYYY-MM-DD.
        control_term_days: Срок контроля в днях (приоритетнее, чем date_control_end).
        control_type_name: Название типа контроля.
        control_employee_id: ФИО или UUID контролёра.
        comment: Комментарий к контролю.
        config: Конфиг LangGraph, передается автоматически (НЕ запрашивать у пользователя).
    """
    try:
