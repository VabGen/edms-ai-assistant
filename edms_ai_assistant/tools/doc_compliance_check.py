...
"""
EDMS AI Assistant — Document Compliance Check Tool (DI Factory).
"""
...
async def _extract_text(
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            ...
            tmp_path = tmp.name
        text = await file_processor.extract_text_async(tmp_path)
...
def create_doc_compliance_check_tool(deps: AppDeps) -> StructuredTool:
    """Фабрика инструмента проверки соответствия полей с DI."""
    ...
    async def doc_compliance_check(
        ...
        - «Проверь на ошибки», «проверь на соответствие», «найди нарушения» — это анализ текста,
          отвечай напрямую из уже полученного текста документа.
        ...
        llm_tasks = [
            _run_llm(category, text_fields, text, name, llm) for name, text in texts
        ]
    ...
    return StructuredTool.from_function(
        ...
        description=(
            ...
            "- «Соответствует ли карточка файлу»\n\n"
            "ЗАПРЕЩЕНО использовать для:\n"
            "- Вопросов о содержимом ранее проанализированного файла.\n"
            "- «Проверь на ошибки», «проверь на соответствие» — это анализ текста, отвечай напрямую.\n\n"
            "ВАЖНО: Токен авторизации и ID документа передаются системой АВТОМАТИЧЕСКИ."
        ),
        args_schema=DocComplianceCheckInput,
    )
