...
from edms_ai_assistant.domain.base import SliceDto
...
class CorrespondentClient(EdmsBaseClient):
    ...
    async def get_correspondents(
        ...
        return await self._request_dto(
            "GET", "api/correspondent", token, SliceDto[CorrespondentDto], params=params
        )

    async def get_correspondent(
        self, token: str, correspondent_id: UUID
    ) -> CorrespondentDto:
    ...
    async def create_correspondent(
        self, token: str, request: CorrespondentAddRequest
    ) -> CorrespondentDto:
        """POST api/correspondent/v2"""
        logger.info("Creating new correspondent")
        result = await self.make_request(
            "POST",
            "api/correspondent/v2",
            token=token,
    ...
    async def update_correspondent(
        self, token: str, request: CorrespondentUpdateRequest
    ) -> CorrespondentDto:
    ...
    async def delete_correspondent(self, token: str, correspondent_id: UUID):
        ...
        await self.make_request(
            "DELETE", f"api/correspondent/{correspondent_id}", token=token
        )
    ...
    async def search_correspondent_fts(self, token: str, fts: str) -> CorrespondentDto:
        ...
        result = await self.make_request(
            "GET", "api/correspondent/fts-name", token=token, params={"fts": fts}
        )
        return CorrespondentDto.model_validate(result)
    ...
    async def import_correspondents(
    ...
    async def get_correspondent_groups(
        self,
        token: str,
        search: BasicSearchRequest | None = None,
        page: int = 0,
        size: int = 20,
        ...
        return await self._request_dto(
            "GET",
            "api/correspondent-group",
            token,
            SliceDto[CorrespondentGroupDto],
            params=params,
        )

    async def get_correspondent_group(
        self, token: str, group_id: UUID
    ) -> CorrespondentGroupDto:
        """GET api/correspondent-group/{id}"""
        logger.info(f"Fetching correspondent group by id: {group_id}")
        result = await self.make_request(
            "GET", f"api/correspondent-group/{group_id}", token=token
        )
        return CorrespondentGroupDto.model_validate(result)

    async def create_correspondent_group(
        self, token: str, request: CorrespondentGroupAddRequest
    ) -> CorrespondentGroupDto:
        """POST api/correspondent-group/v2"""
        logger.info("Creating new correspondent group")
        result = await self.make_request(
            "POST",
            "api/correspondent-group/v2",
            token=token,
            json_data=request.model_dump(exclude_none=True),
        )
        return CorrespondentGroupDto.model_validate(result)

    async def update_correspondent_group(
        self, token: str, request: CorrespondentGroupUpdateRequest
    ) -> CorrespondentGroupDto:
