...
class AttachmentClient(EdmsBaseClient):
    ...
    async def get_attachment_content(
        ...
        path = "additional-attachment" if is_additional else "attachment"
        logger.info(
            "Downloading content of attachment %s for document %s",
            attachment_id,
            document_id,
        )
        result = await self.make_request(
    ...
    async def get_source_content(
        self, token: str, document_id: UUID, attachment_id: UUID
    ) -> bytes:
    ...
    async def get_attachment_stamp(
        self, token: str, document_id: UUID, attachment_id: UUID
    ) -> bytes:
    ...
    async def rename_attachment(
        self, token: str, document_id: UUID, attachment_id: UUID, name: str
    ) -> AttachmentDocumentDto:
    ...
    async def change_type(
        ...
        document_id: UUID,
        attachment_id: UUID,
        request: ChangeAttachmentDocumentTypeRequest,
    ) -> AttachmentDocumentDto:
    ...
    async def convert_all_to_pdf(
        self, token: str, document_id: UUID, attachment_ids: list[UUID]
    ):
    ...
    async def upload_print_document(
        ...
        result = await self.make_request(
            "POST",
            f"api/document/{document_id}/print-document",
            token=token,
            files={"file": (filename, file_content)},
        )
        return AttachmentDocumentDto.model_validate(result)

    async def upload_project_solution(
        self, token: str, document_id: UUID, file_content: bytes, filename: str
    ) -> AttachmentDocumentDto:
        """POST api/document/{documentId}/project-solution"""
        result = await self.make_request(
            "POST",
            f"api/document/{document_id}/project-solution",
            token=token,
            files={"file": (filename, file_content)},
        )
        return AttachmentDocumentDto.model_validate(result)

    async def upload_rationale(
        self, token: str, document_id: UUID, file_content: bytes, filename: str
    ) -> AttachmentDocumentDto:
        """POST api/document/{documentId}/rationale"""
        result = await self.make_request(
            "POST",
            f"api/document/{document_id}/rationale",
            token=token,
            files={"file": (filename, file_content)},
        )
        return AttachmentDocumentDto.model_validate(result)

    async def upload_documents_question(
        self, token: str, document_id: UUID, file_content: bytes, filename: str
    ) -> AttachmentDocumentDto:
        """POST api/document/{documentId}/documents-question"""
        result = await self.make_request(
            "POST",
            f"api/document/{document_id}/documents-question",
            token=token,
            files={"file": (filename, file_content)},
        )
        return AttachmentDocumentDto.model_validate(result)
    ...
    async def upload_decision(
        ...
        result = await self.make_request(
            "POST",
            f"api/document/{document_id}/decision",
            token=token,
            files={"file": (filename, file_content)},
        )
        return AttachmentDocumentDto.model_validate(result)
    ...
    async def create_empty(
        self, token: str, document_id: UUID, request: CreateEmptyFileRequest
    ) -> AttachmentDocumentDto:
    ...
    async def add_sign(
        self, token: str, document_id: UUID, attachment_id: UUID, cms: str
    ) -> AttachmentSignature:
    ...
    async def remove_sign(
        self, token: str, document_id: UUID, attachment_id: UUID, sign_id: UUID
    ):
    ...
    async def verify_sign(
        ...
        document_id: UUID,
        attachment_id: UUID,
        request: CheckAttachmentSignRequest,
    ) -> dict[str, Any]:
    ...
    async def upload_temporary_template(
        self, token: str, file_content: bytes, filename: str
    ) -> TemporaryAttachmentDto:
    ...
    async def upload_temporary_for_document(
        ...
        """
        mapping = {
            AttachmentDocumentType.ATTACHMENT: (
                "additional-attachment" if is_additional else ""
            ),
    ...
    async def get_temporary_mini_doc_base64(
        self, token: str, attachment_id: UUID
    ) -> bytes:
    ...
    async def sign_temporary_mini_doc(
        self, token: str, attachment_id: UUID, cms: str
    ) -> Any:
    ...
    async def remove_temporary_mini_doc_sign(
        self, token: str, attachment_id: UUID, sign_id: UUID
    ):
