...
class PermissionClient(EdmsBaseClient):
    ...
    async def get_permission(self, token: str, permission_id: str | UUID) -> PermissionDto:
        """Fetches permission by ID."""
        logger.info(f"Fetching permission {permission_id}")
        return await self._request_dto("GET", f"api/permission/{permission_id}", token, PermissionDto)
    ...
    async def get_definition(self, token: str, system_name: str) -> dict[str, Any]:
        ...
        return await self.make_request("GET", f"api/permission/{system_name}/definition", token)

    async def add_role(self, token: str, permission_id: str | UUID, role_id: str | UUID) -> None:
    ...
    async def add_roles_batch(self, token: str, permission_id: str | UUID, role_ids: list[UUID]) -> None:
    ...
    async def remove_role(self, token: str, permission_id: str | UUID, role_id: str | UUID) -> None:
    ...
    async def remove_roles_batch(self, token: str, permission_id: str | UUID, role_ids: list[UUID]) -> None:
    ...
    async def copy_permission(self, token: str, permission_id: str | UUID, copy_request: dict[str, Any]) -> list[PermissionDto]:
    ...
    async def reload_permissions(self, token: str) -> None:
        ...
        await self.make_request("POST", "api/permission/reload", token, is_json_response=False)
    ...
    async def update_permission(self, token: str, permission: PermissionDto) -> PermissionDto:
        """Updates a permission entry."""
        logger.info(f"Updating permission {permission.id}")
        return await self._request_dto(
            "PUT", "api/permission", token, PermissionDto, json_data=permission.model_dump(by_alias=True)
        )
    ...
    async def get_by_system_name(
        ...
        summary_nomenclature_department_status: SummaryNomenclatureDepartmentStatus | None = None,
        ...
        if nomenclature_department_status: params["nomenclatureDepartmentStatus"] = nomenclature_department_status
        if summary_nomenclature_department_status: params["summaryNomenclatureDepartmentStatus"] = summary_nomenclature_department_status
        if destruction_act_status: params["destructionActStatus"] = destruction_act_status
        if acceptance_inventory_status: params["acceptanceInventoryStatus"] = acceptance_inventory_status

        return await self._request_list("GET", "api/permission/system-name", token, PermissionDto, params=params)
    ...
    async def create_by_system_name(
        ...
        return await self._request_dto("POST", "api/permission/system-name", token, PermissionDto, params=params)

    async def get_permission_roles(self, token: str, permission_id: str | UUID) -> list[PermissionRoleDto]:
        """Fetches roles for a permission."""
        logger.info(f"Fetching roles for permission {permission_id}")
        return await self._request_list("GET", f"api/permission/{permission_id}/role", token, PermissionRoleDto)
    ...
    async def import_permissions(self, token: str, import_definition: dict[str, Any]) -> None:
        """Imports permission settings."""
        logger.info("Importing permissions")
        await self.make_request("POST", "api/permission/import", token, json_data=import_definition, is_json_response=False)
