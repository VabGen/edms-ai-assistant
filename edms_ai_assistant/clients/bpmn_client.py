...
class BpmnClient(EdmsBaseClient):
    ...
    async def upload(self, token: str, request: BpmnProcessDirectoryRequest) -> BpmnProcessDirectoryDto:
        """POST api/bpmn"""
        return await self._request_dto("POST", "api/bpmn", token, BpmnProcessDirectoryDto, json_data=request.model_dump(exclude_none=True))
    ...
    async def get_all(self, token: str, request: BpmnSearchRequest | None = None, page: int = 0, size: int = 20) -> SliceDto[BpmnProcessDirectoryDto]:
        """GET api/bpmn"""
        params = request.model_dump(exclude_none=True) if request else {}
        params.update({"page": page, "size": size})
        return await self._request_dto("GET", "api/bpmn", token, SliceDto[BpmnProcessDirectoryDto], params=params)

    async def get_by_id(self, token: str, bpmn_id: UUID) -> BpmnProcessDirectoryDto:
        """GET api/bpmn/{id}"""
        return await self._request_dto("GET", f"api/bpmn/{bpmn_id}", token, BpmnProcessDirectoryDto)

    async def edit(self, token: str, request: BpmnProcessDirectoryRequest) -> BpmnProcessDirectoryDto:
        """PUT api/bpmn"""
        return await self._request_dto("PUT", "api/bpmn", token, BpmnProcessDirectoryDto, json_data=request.model_dump(exclude_none=True))

    async def copy(self, token: str, ids: list[UUID]) -> list[BpmnProcessDirectoryDto]:
        """POST api/bpmn/copy"""
        return await self._request_list("POST", "api/bpmn/copy", token, BpmnProcessDirectoryDto, json_data={"ids": [str(i) for i in ids]})

    async def delete(self, token: str, bpmn_id: UUID) -> None:
        """DELETE api/bpmn/{id}"""
        await self.make_request("DELETE", f"api/bpmn/{bpmn_id}", token=token, is_json_response=False)

    async def delete_list(self, token: str, ids: list[UUID]) -> None:
        """DELETE api/bpmn"""
        await self.make_request("DELETE", "api/bpmn", token=token, json_data={"ids": [str(i) for i in ids]}, is_json_response=False)
