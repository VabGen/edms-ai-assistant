...
class AccessGriefClient(EdmsBaseClient):
    ...
    async def get_grief(self, token: str, grief_id: str | UUID) -> AccessGriefDto | None:
    ...
    async def create_grief(self, token: str, request: AccessGriefRequest) -> AccessGriefDto:
        """Creates a new access grief."""
        logger.info("Creating new access grief")
        return await self._request_dto(
            "POST", "api/access-grief", token, AccessGriefDto, json_data=request.model_dump(by_alias=True)
        )

    async def update_grief(self, token: str, grief_id: str | UUID, request: AccessGriefRequest) -> AccessGriefDto:
        """Updates an existing access grief."""
        logger.info(f"Updating access grief {grief_id}")
        return await self._request_dto(
            "PUT", f"api/access-grief/{grief_id}", token, AccessGriefDto, json_data=request.model_dump(by_alias=True)
        )
    ...
    async def delete_grief(self, token: str, grief_id: str | UUID) -> None:
        ...
        await self.make_request("DELETE", f"api/access-grief/{grief_id}", token, is_json_response=False)
    ...
    async def delete_griefs(self, token: str, grief_ids: list[UUID]) -> None:
        ...
        await self.make_request("DELETE", "api/access-grief", token, json_data={"ids": grief_ids}, is_json_response=False)
