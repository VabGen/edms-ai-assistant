...
class TaskClient(EdmsBaseClient):
    ...
    async def get_all_tasks(
        ...
        return await self._request_dto(
            "GET", "api/task", token, SliceDto[TaskDto], params=params
        )
    ...
    async def get_stat_executor(self, token: str) -> ExecutionTaskStatCount:
        ...
        return await self._request_dto(
            "GET", "api/task/stat/user-executor", token, ExecutionTaskStatCount
        )
    ...
    async def get_stat_control(self, token: str) -> ExecutionTaskStatCount:
        ...
        return await self._request_dto(
            "GET", "api/task/stat/user-control", token, ExecutionTaskStatCount
        )
    ...
    async def get_stat_author(self, token: str) -> ExecutionTaskStatCount:
        ...
        return await self._request_dto(
            "GET", "api/task/stat/user-author", token, ExecutionTaskStatCount
        )

    async def get_stat_period(
        ...
        return await self._request_dto(
            "GET",
            "api/task/stat/execution-period",
            token,
            TaskExecutionStatByPeriod,
            params=params,
        )
    ...
    async def create_kanban_column(
        self, token: str, request: TaskKanbanColumnDto
    ) -> TaskKanbanColumnDto:
    ...
    async def update_kanban_column(
        self, token: str, column_id: str | UUID, request: TaskKanbanColumnDto
    ) -> TaskKanbanColumnDto:
    ...
    async def change_column_order(self, token: str, column_ids: list[UUID]) -> None:
        ...
        await self.make_request(
            "POST",
            "api/task/kanban/change-order",
            token,
            json_data={"ids": column_ids},
            is_json_response=False,
        )

    async def change_task_order_in_column(
        self, token: str, column_id: str | UUID, task_keys: list[OrgKey]
    ) -> None:
    ...
    async def get_document_tasks(
        self, token: str, document_id: str | UUID
    ) -> list[TaskDto]:
        """Fetches all tasks associated with a document."""
        logger.info(f"Fetching tasks for document {document_id}")
        return await self._request_list(
            "GET", f"api/document/{document_id}/task", token, TaskDto
        )

    async def create_task(
        self, token: str, document_id: str | UUID, request: CreateTaskRequest
    ) -> TaskDto:
    ...
    async def update_task(
        self, token: str, document_id: str | UUID, request: UpdateTaskRequest
    ) -> TaskDto:
    ...
    async def delete_task(
        self, token: str, document_id: str | UUID, task_id: str | UUID
    ) -> None:
    ...
    async def get_task(
        ...
        return await self._request_dto(
            "GET", f"api/document/{document_id}/task/{task_id}", token, TaskDto
        )
    ...
    async def change_responsible(
        ...
        logger.info(
            f"Changing responsibility for {executor_id} in task {task_id} to {status}"
        )
        return await self._request_dto(
            "PUT",
    ...
    async def get_task_projects(
        ...
        if filter_data:
            params.update(filter_data)
        return await self._request_dto(
            "GET", "api/task-project", token, SliceDto[TaskProjectDto], params=params
        )
