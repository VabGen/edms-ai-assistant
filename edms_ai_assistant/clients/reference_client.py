...
class ReferenceClient(EdmsHttpClient):
    ...
    async def _find_entity_with_name(
        ...
        if not search_name or not search_name.strip():
            logger.debug(f"[REFERENCE-CLIENT] Пропуск поиска {entity_label}: пустое значение")
            return None
        ...
        try:
            ...
            if not result:
                logger.warning(f"[REFERENCE-CLIENT] {entity_label} не найден: '{search_query}'")
                return None

            data = result[0] if isinstance(result, list) and len(result) > 0 else (
                result if isinstance(result, dict) else None
            )

            if not data:
                logger.warning(f"[REFERENCE-CLIENT] Пустой ответ для {entity_label}: '{search_query}'")
                return None

            entity_id = str(data.get("id"))

            entity_name = (
                    data.get(name_field) or
    ...
    async def find_district_with_name(
        ...
        return await self._find_entity_with_name(
            token, "district", name, "Район",
            name_field="districtName"
        )
    ...
    async def find_city_with_hierarchy(
        ...
        if region_id:
            try:
                region_dto = await self._make_request(
                    "GET",
                    f"api/region/{region_id}",
                    token=token
                )
                response["regionId"] = str(region_id)
                response["regionName"] = region_dto.get("regionName") or region_dto.get("name")
                logger.info(f"[REFERENCE-CLIENT] Регион для '{city_name}': {response['regionName']}")
            except Exception as e:
        ...
        if district_id:
            try:
                ...
                response["districtName"] = district_dto.get("districtName") or district_dto.get("name")
                logger.info(f"[REFERENCE-CLIENT] Район для '{city_name}': {response['districtName']}")
            except Exception as e:
...
Ответь ТОЛЬКО номером (например: 3)"""
        ...
        try:
            ...
            if not match:
                logger.warning(f"[REFERENCE-CLIENT] LLM не вернула номер: {choice_text}")
                return None
            ...
            logger.info(f"[REFERENCE-CLIENT] Родительская тема: {parent['name']} (ID: {parent_id})")
...
Ответь ТОЛЬКО номером (например: 2)"""
            ...
            if not match2:
                logger.warning(f"[REFERENCE-CLIENT] LLM не вернула номер подтемы: {choice2_text}")
                return parent_id

            child_index = int(match2.group(0)) - 1
            if child_index < 0 or child_index >= len(children):
                logger.warning(f"[REFERENCE-CLIENT] Неверный индекс подтемы: {child_index}")
                return parent_id

            child = children[child_index]
            child_id = str(child['id'])
            logger.info(f"[REFERENCE-CLIENT] Дочерняя тема: {child['name']} (ID: {child_id})")
