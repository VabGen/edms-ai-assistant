...
class EntityExtractor:
    ...
    def extract_dates(
        self, text: str, base_date: datetime | None = None
    ) -> list[Entity]:
            ...
            for match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                try:
                    ...
                    if handler == "month_name":
                        day, month = (
                            int(match.group(1)),
                            self.MONTH_NAMES[match.group(2)],
                        )
    ...
    def extract_persons(self, text: str) -> list[Entity]:
        ...
        for match in re.finditer(
            r"\b([А-ЯЁ][а-яё]+)\s+([А-ЯЁ][а-яё]+)(?:\s+([А-ЯЁ][а-яё]+))?\b", text
        ):
            last_name, first_name = match.group(1), match.group(2)
            if (
                last_name.lower() in self._PERSON_STOP_WORDS
                or first_name.lower() in self._PERSON_STOP_WORDS
            ):
                continue
            persons.append(
                Entity(
    ...
    def extract_numbers(text: str) -> list[Entity]:
        return [
            Entity(
                ...
                raw_text=m.group(0),
                normalized_value=float(m.group(0).replace(",", ".")),
            )
