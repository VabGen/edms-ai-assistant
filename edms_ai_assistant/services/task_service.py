...
class TaskService:
    ...
    async def create_task(
        ...
        emp_ids, not_found, ambiguous = await self._resolution.resolve_employees(
            token, executor_last_names
        )
    ...
    async def create_task_by_employee_ids(
        ...
        return await self._submit_task(
            token, document_id, task_text, executors, planed_date_end, task_type
        )
    ...
    async def collect_executors(
        ...
        responsible_id: UUID | None = None
        if responsible_last_name:
            resp_ids, _, _ = await self._resolution.resolve_employees(
                token, [responsible_last_name]
            )
            if resp_ids:
    ...
    async def _submit_task(
        ...
        try:
            ...
            return TaskCreationResult(
                success=False, status="error", error_message=f"Ошибка API: {exc}"
            )
