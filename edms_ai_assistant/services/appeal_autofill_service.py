...
class AttachmentSelector:
    ...
    @classmethod
    def select(
        cls, document: DocumentDto, attachment_id: str | None
    ) -> tuple[AttachmentDocumentDto, list[str]]:
        warnings = []
        ...
        if attachment_id:
            target = next(
                (a for a in attachments if str(a.id) == attachment_id),
                None,
            )
            if not target:
                warnings.append(
...
class GeographyResolver:
    ...
    async def resolve_geography(
        self, document: DocumentDto, fields: AppealFields
    ) -> dict[str, Any]:
        ...
        district_name = self._get_field(
            document, fields, "district_name", "districtName"
        )
    ...
    @staticmethod
    def _get_field(
        document: DocumentDto, fields: AppealFields, snake_attr: str, camel_attr: str
    ) -> str | None:
    ...
    async def _resolve_country(
        self, document: DocumentDto, fields: AppealFields, geo_data: dict[str, Any]
    ) -> None:
        d = document.document_appeal
    ...
    async def _resolve_city(
        self, city_name: str | None, document: DocumentDto, geo_data: dict[str, Any]
    ) -> None:
        d = document.document_appeal
        if city_name:
            try:
...
class AppealFieldsBuilder:
    ...
    async def _add_correspondent(
        self, d: DocumentAppealDto, fields: AppealFields, payload: dict[str, Any]
    ) -> None:
        ...
        if corr_name:
            ...
            if canonical and _is_good_correspondent_match(
                corr_name, canonical.name or ""
            ):
                payload["correspondentAppealId"] = (
                    str(canonical.id) if canonical.id else None
                )
                payload["correspondentAppeal"] = ValueSanitizer.sanitize_string(
                    corr_name
                )
    ...
    def _add_personal_data(
        d: DocumentAppealDto, fields: AppealFields, payload: dict[str, Any]
    ) -> None:
    ...
    async def _add_conditional_fields(
        ...
        if payload.get("declarantType") == DeclarantType.ENTITY:
            raw_org: str | None = (
                str(d.organization_name)
                if not ValueSanitizer.is_empty(d.organization_name)
                else fields.organizationName
            )
            if not ValueSanitizer.is_empty(raw_org):
                corr_data = await self.ref_client.find_entity_with_name(
                    self.token, "correspondent", str(raw_org), "Организация"
                )
                if corr_data and _is_good_correspondent_match(
                    str(raw_org), corr_data.name or ""
                ):
            ...
            payload["signed"] = ValueSanitizer.sanitize_string(
                d.signed or fields.signed
            )
            payload["correspondentOrgNumber"] = ValueSanitizer.sanitize_string(
                d.correspondent_org_number or fields.correspondentOrgNumber
            )
    ...
    def _add_common_fields(
        ...
        payload["collective"] = (
            ...
            else (d.collective or False)
        )
        payload["anonymous"] = (
            fields.anonymous if fields.anonymous is not None else (d.anonymous or False)
        )
        payload["reasonably"] = (
            fields.reasonably
            if fields.reasonably is not None
            else (d.reasonably or False)
        )

        raw_receipt = d.receipt_date or fields.receiptDate
        payload["receiptDate"] = ValueSanitizer.fix_datetime_format(
            raw_receipt
        ) or datetime.now(UTC).strftime("%Y-%m-%dT00:00:00Z")

        payload["fullAddress"] = ValueSanitizer.sanitize_string(
            d.full_address or fields.fullAddress
        )
        ...
        payload["submissionForm"] = (
            sub_form if sub_form else SubmissionFormAppeal.WRITTEN
        )
    ...
    def _filter_payload(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        filtered = {
            k: v for k, v in payload.items() if v is not None or k in _ALLOW_NULL
        }
...
class AppealAutofillService:
    ...
    async def process_and_fill(
        ...
        if document.doc_category_const != DocCategory.APPEAL:
            raise ValueError(
                f"Документ должен быть категории APPEAL, а не {document.doc_category_const}"
            )
        ...
        await self._update_document(
            token, document_id, document, fields, extracted_text
        )

        summary_choices = None
        if generate_summary_choices:
            summary_choices = await self.generate_summary_variants(
                extracted_text, fields.shortSummary
            )
    ...
    async def _extract_text(
        self, token: str, document_id: str, attachment: AttachmentDocumentDto
    ) -> str:
        if not attachment.id:
            return ""
        file_bytes = await self.attach_client.get_attachment_content(
            token, document_id, attachment.id
        )
        if not file_bytes:
            return ""
    ...
    async def _execute_main_fields_update(
        ...
        main_payload = {
            ...
            "documentTypeId": (
                str(document.document_type.id) if document.document_type else None
            ),
        }
        await DocumentOperationExecutor.execute(
            self.doc_client,
    ...
    async def generate_summary_variants(
        self, text: str, current_summary: str | None
    ) -> list[str]:
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "Сформулируй РОВНО 3 варианта краткого содержания обращения (до 80 симв).",
                ),
                ("user", "Текст:\n{text}"),
            ]
        )
