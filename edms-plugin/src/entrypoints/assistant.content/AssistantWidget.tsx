...
function getToastErrorText(err: unknown): string {
    ...
    if (err instanceof Error) rawMsg = err.message
        ...
        const e = err as Record<string, unknown>
        rawMsg = typeof e.error === 'string' ? e.error : typeof e.message === 'string' ? e.message : typeof e.detail === 'string' ? e.detail : JSON.stringify(e)
    } else {
        rawMsg = String(err)
    }
...
const CandidateIcon = memo(({type, className}: { type: string; className?: string }) => {
    ...
    return (<svg className={cls} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
    ...
    return (
        <div className="mt-2 flex flex-wrap gap-2">
            {[{id: 'abstractive', label: 'Пересказ', icon: <FileText size={13}/>}, {
                ...
                label: 'Факты',
                icon: <Search size={13}/>
            }, {id: 'thesis', label: 'Тезисы', icon: <List size={13}/>}].map(btn => (
                <button key={btn.id} type="button" disabled={loading}
                        onClick={e => {
                            e.stopPropagation();
...
export function AssistantWidget() {
    ...
    const send = async (e?: FormEvent | React.KeyboardEvent, humanChoice?: string, humanChoiceLabel?: string) => {
        ...
        const userLabel = isChoiceFlow ? (humanChoiceLabel ?? CHOICE_LABELS[humanChoice!] ?? humanChoice!) : hasFile ? `${input} (Файл: ${attachedFile!.name})`.trim() : input
    ...
    const refreshDocumentPage = async (_documentId: string | null) => {
        ...
        const snapshot = {messages: messagesRef.current, threadId, isOpen: true, savedAt: Date.now()}
        try {
                ...
                <div
                    ...
                    <header className="flex items-center justify-between px-4 py-3 shrink-0"
                        ...
                        <div className="flex items-center gap-2">
                            <button type="button" onClick={() => setIsSidebarOpen(v => !v)}
                    ...
                    <div className="flex-1 flex overflow-hidden">
                                        ...
                                        <div className="flex-1 overflow-y-auto scrollbar-thin flex flex-col gap-1.5">
                                            {threads.length === 0 ? (
                                                ...
                                                <div key={t.id}
                                                    ...
                                                    <button type="button" onClick={(e) => deleteThread(t.id, e)}
                                                            title="Удалить"
                        ...
                        <main className="flex-1 flex flex-col min-w-0 overflow-hidden"
                            ...
                            <div className="flex-1 p-4 overflow-y-auto scrollbar-thin flex flex-col gap-3">
                                ...
                                {messages.map(msg => {
                                    const displayContent = msg.action_type === 'requires_disambiguation' ? cleanDisambigMessage(msg.content) : msg.content
                                    return (
                            ...
                            <footer className="px-3 pb-3 pt-2 shrink-0" style={{
                                ...
                                {attachedFile && (
                                    <div className="flex items-center gap-2 mb-2 px-3 py-1.5 rounded-xl w-fit"
                                         style={{
                                ...
                                <div style={{
                                    ...
                                    {isSpeechSupported && (
                                        <>
                                            <button type="button" title={isListening ? 'Остановить' : 'Голосовой ввод'}
                                                    onClick={toggleMic}
                                            ...
                                            {(isListening || handsFree) && (
                                                <button type="button" onClick={() => setHandsFree(v => !v)}
