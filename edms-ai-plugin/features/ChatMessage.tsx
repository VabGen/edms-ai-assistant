        ...
        <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            ...
            >
                <ReactMarkdown
                    ...
                    components={{
                        // Кастомный рендер блоков кода
                        code({ node, inline, className, children, ...props }: any) {
                            const match = /language-(\w+)/.exec(className || '');
                            if (!inline) {
                                return (
                                    <div className="relative my-3">
                                        <pre className="overflow-x-auto p-3 rounded-xl bg-slate-900 font-mono text-xs text-indigo-100 shadow-inner">
                        ...
                        // Цитаты
                        blockquote: ({ children }) => (
                            <blockquote className={`border-l-4 pl-3 italic my-2 ${isUser ? 'border-indigo-300 text-indigo-100' : 'border-slate-200 text-slate-500'}`}>
                                {children}
                            </blockquote>
                        ),

                        // Таблицы (нужны для remarkGfm)
                        table: ({ children }) => (
                        ...
                        th: ({ children }) => <th className="bg-slate-50 p-2 border border-slate-100 font-bold">{children}</th>,
